...
**Slice 1 (read-only), plugin 0.1.6:** config flow, fleet sensors, one HA device per host. **Visit** on the host opens that server in PiHerder. Nested **Docker / Backups / Alerts / Audit** devices each **Visit** that page (real links, not press-here buttons). Token scope **`read`**. Poll is database snapshots — never SSH the fleet.
...
HACS does **not** auto-refresh custom repos. New release: HACS → PiHerder → **⋮ → Redownload** → pick the tag → **restart Home Assistant**. Confirm fleet **Plugin** is **0.1.6**.
